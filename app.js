...
function switchTab(tabName) {
    ...
    else if (tabName === 'home') { renderHome(); document.getElementById('home-view').classList.remove('hidden'); }
    else if (tabName === 'accounts') { renderAccounts(); document.getElementById('accounts-view').classList.remove('hidden'); }
    else if (tabName === 'settings') { document.getElementById('settings-view').classList.remove('hidden'); }
    else if (tabName === 'graph') { renderGraph(); document.getElementById('graph-view').classList.remove('hidden'); }
}
...
function renderHome() {
    ...
    if (accountSelect.options.length === 0) { accountSelect.innerHTML = '<option value="all">All Accounts</option>'; categories.filter(c => c.type === 'account').forEach(acc => { accountSelect.innerHTML += `<option value="${acc.id}">${acc.name}</option>`; }); }
    const selectedAccountId = accountSelect.value;
    
    if (selectedAccountId === 'all') { let total = 0; categories.filter(c => c.type === 'account').forEach(acc => total += parseFloat(getAccountBalance(acc.id))); document.getElementById('home-balance').innerText = `$${total.toFixed(2)}`; }
    else { document.getElementById('home-balance').innerText = `$${getAccountBalance(selectedAccountId)}`; }
    
    // Renders
    renderHomeBudgets(); // NEW: Render Budgets on Home
    updateChart(selectedAccountId);
}

// NEW FUNCTION: RENDER BUDGETS ON HOME
function renderHomeBudgets() {
    const container = document.getElementById('budget-overview');
    container.innerHTML = '';
    
    // Filter Categories with Caps
    ...
    header.innerText = "Monthly Budgets";
...
function updateChart(accountId) {
    ...
    sortedTrans.forEach(t => {
        let name, icon, color;
        if (!t.categoryId) { name = t.type === 'income' ? 'Income' : 'Uncategorized'; icon = t.type === 'income' ? '💵' : '❓'; color = t.type === 'income' ? '#03dac6' : '#888'; }
        else { const cat = categories.find(c => c.id == t.categoryId); name = cat ? cat.name : 'Unknown'; icon = cat ? cat.icon : '❓'; color = cat ? cat.color : '#888'; }
        const row = document.createElement('div'); row.className = 'spending-item';
        row.innerHTML = `<div class="spending-left"><div class="spending-icon" style="background-color:${color}20; color:${color}">${icon}</div><div class="spending-info"><span class="spending-name">${name}</span><span class="spending-pct" style="font-size:10px">${t.date} ${t.tags && t.tags.length > 0 ? '• ' + t.tags.join(', ') : ''}</span></div></div><div class="spending-amount" style="color:${color}">$${parseFloat(t.amount).toFixed(2)}</div>`;
...
function renderGraph() {
    ...
    document.getElementById('graph-total-amount').innerText = `$${grandTotal.toFixed(2)}`;
...
// --- CATEGORIES RENDER (KEEPING FOR CAT TAB) ---
function renderCategories() {
    ...
    categories.forEach(cat => {
        ...
        if (cat.type === 'section') {
            transactions.forEach(t => {
                if (t.categoryId == cat.id && t.type === 'expense' && t.date.startsWith(currentMonth)) {
                    monthlySpent += parseFloat(t.amount);
                }
            });
        }

        let barHtml = '';
        if (cat.type === 'section' && cat.cap > 0) {
            const pct = Math.min((monthlySpent / cat.cap) * 100, 100);
            let colorClass = 'prog-green';
            if (pct >= 90) colorClass = 'prog-red'; else if (pct >= 50) colorClass = 'prog-orange';
            barHtml = `<div style="margin-top:5px;"><div style="display:flex; justify-content:space-between; font-size:10px; color:#888; margin-bottom:2px;"><span>Spent: $${monthlySpent.toFixed(0)}</span><span>Cap: $${cat.cap}</span></div><div class="budget-bar-track"><div class="budget-bar-fill ${colorClass}" style="width:${pct}%"></div></div></div>`;
        }

        item.innerHTML = `<div class="cat-icon" style="background-color: ${cat.color}20; color: ${cat.color}">${cat.icon}</div><div class="cat-info"><div style="font-weight:bold; color:#fff">${cat.name}</div><div style="font-size:12px; color:#888">${cat.type.toUpperCase()}</div>${barHtml}</div><button class="edit-btn-icon" onclick="editCategory(${cat.id})">✏️</button>`;
...
function renderAccounts() {
    ...
    accs.forEach(acc => { const bal = getAccountBalance(acc.id); sum += parseFloat(bal); const card = document.createElement('div'); card.className = 'account-card'; card.style.borderLeftColor = acc.color; card.innerHTML = `<div class="acc-details"><div class="acc-icon">${acc.icon}</div><div class="acc-name">${acc.name}</div></div><div style="display:flex; align-items:center;"><div class="acc-balance" style="margin-right:10px;">$${bal}</div><button class="edit-btn-icon" onclick="editCategory(${acc.id})">✏️</button></div>`; list.appendChild(card); });
    gt.innerText = `$${sum.toFixed(2)}`;
}

// --- CATEGORY FORM ---
function openCategoryForm() { editingId = null; document.getElementById('modal-title').innerText = "New Category"; document.getElementById('cat-name').value = ''; document.getElementById('cat-initial-balance').value = ''; document.getElementById('cat-cap').value = ''; selectedType = 'account'; selectedColor = colors[0]; selectedIcon = icons[0]; updateFormUI(); document.getElementById('category-form-overlay').classList.remove('hidden'); }
function editCategory(id) { const c = categories.find(x => x.id === id); if(!c) return; editingId = id; document.getElementById('modal-title').innerText = "Edit Category"; document.getElementById('cat-name').value = c.name; document.getElementById('cat-initial-balance').value = c.initialBalance || ''; document.getElementById('cat-cap').value = c.cap || ''; selectedType = c.type; selectedColor = c.color; selectedIcon = c.icon; updateFormUI(); document.getElementById('category-form-overlay').classList.remove('hidden'); }
function updateFormUI() { document.querySelectorAll('.type-btn').forEach(b => { b.classList.remove('active'); if (b.innerText.toLowerCase() === selectedType) b.classList.add('active'); }); const balInput = document.getElementById('balance-input-container'); const capInput = document.getElementById('cap-input-container'); if (selectedType === 'account') { balInput.classList.remove('hidden'); capInput.classList.add('hidden'); } else { balInput.classList.add('hidden'); capInput.classList.remove('hidden'); } setupPickers(); }
function saveCategory() { const name = document.getElementById('cat-name').value.trim(); if (!name) return alert('Name required'); const data = { id: editingId ? editingId : Date.now(), name, type: selectedType, color: selectedColor, icon: selectedIcon, initialBalance: selectedType === 'account' ? (parseFloat(document.getElementById('cat-initial-balance').value) || 0) : 0, cap: selectedType === 'section' ? (parseFloat(document.getElementById('cat-cap').value) || 0) : 0 }; if (editingId) { const idx = categories.findIndex(c => c.id === editingId); if (idx !== -1) categories[idx] = data; } else { categories.push(data); } localStorage.setItem('categories', JSON.stringify(categories)); renderCategories(); renderAccounts(); renderHome(); closeCategoryForm(); }
...
function openTransactionForm() { editingTxId = null; document.getElementById('tx-modal-title').innerText = "Add Transaction"; document.getElementById('btn-delete-tx').classList.add('hidden'); tagDeleteMode = false; updateTagActionBtn(); setFormType(viewMode); populateSelects(); currentTxTags = []; document.getElementById('tx-tag-input').value = ''; renderTagCloud(); document.getElementById('tx-date').valueAsDate = new Date(); document.getElementById('tx-amount').value = ''; document.getElementById('transaction-form-overlay').classList.remove('hidden'); }
...
function addNewTag() { const input = document.getElementById('tx-tag-input'); const val = input.value.trim(); if(val) { if(!allTags.includes(val)) { allTags.push(val); localStorage.setItem('allTags', JSON.stringify(allTags)); } if(!currentTxTags.includes(val)) { currentTxTags.push(val); } input.value = ''; renderTagCloud(); } }
